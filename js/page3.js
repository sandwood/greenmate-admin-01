...
function get_numberof_plants() {
  $.ajax({
    type: "POST",
    url: "http://www.nextfarmer.net/plant/getAllPlants",
    data: {
      category: "all"
    }
    ...
    success: function(data, textStatus, jqXHR) {
      ...
      for (var i = 0; i < data.plants.length; i++) {
        ...
        $("#plant_").append(
          `
					<tr class="plant_">
						<td><div class="chip transparent plant_checkbox" id="` +
            position +
            `"><i class="material-icons">library_add</i></div></td>
						<td class="plant_id">` +
            position +
            `</td>
						<td id="place_` +
            ...
            updated_at +
            `</td>
						<td class="hidden_plant_id" style="display:none">` +
            plantId +
            `</td>
					</tr>`
        );
...
function start_plants_search() {
  ...
  $.ajax({
    type: "POST",
    url: "http://www.nextfarmer.net/search/searchPlant",
    data: {
      keyword: keyword_,
      page: 1,
      pageNumber: 5
...
function start_plant_mine() {
  ...
  $.ajax({
    type: "POST",
    url: "http://www.nextfarmer.net/manager/managingPlants",
    headers: {
      userSeq: userSeq
    },
    data: {
...
function goto_inner_plant(plantId) {
  ...
  $.ajax({
    type: "POST",
    url: "http://www.nextfarmer.net/manager/plantDetail",
    data: {
      plantId: plantId
    },
    success: function(data, textStatus, jqXHR) {
      ...
      for (var i = 0; i < data.diaries.length; i++) {
        ...
        $("#plant_diaries").append(
          `
            ...
            data.diaries[i].published_date +
            `</p>
							<img class="lazy" data-original="` +
            data.diaries[i].picUrl +
            `">
							<p class="col s12">` +
            data.diaries[i].content +
            `</p>
						</div>
				`
        );
...
function delete_plant() {
  ...
  if (del == true) {
    ...
    $.ajax({
      type: "PUT",
      url: "http://www.nextfarmer.net/delete/deletePlant",
      headers: {
        userSeq: userSeq
      },
      data: {
...
function plant_checkbox() {
  $(".plant_checkbox").click(function() {
    ...
    var seller = $("#plant_manager" + target).html();
    var buyer = $("#plant_username" + target).html();

    $("#qr_checker").append(
      `
			<div class="chip" id="qr_check_` +
        target +
        `">
				` +
        target +
        `
				...
				<span class="place_chip" style="display:none">` +
        place +
        `</span>
				<span class="plantname_chip" style="display:none">` +
        plantname +
        `</span>
				<span class="date_chip" style="display:none">` +
        date +
        `</span>
				<span class="plant_manager_chip" style="display:none">` +
        seller +
        `</span>
				<span class="plant_username_chip" style="display:none">` +
        buyer +
        `</span>
			</div>
		`
...
function get_userIds() {
  ...
  $.ajax({
    type: "POST",
    url: "http://www.nextfarmer.net/search/searchUser",
    headers: {
      userSeq: userSeq
    },
    data: {
    ...
    success: function(data, textStatus, jqXHR) {
      console.log(data);
      for (var i = 0; i < data.userId.ids.length; i++) {
        ids_[i] = data.userId.ids[i];
        usernames_[i] = data.userId.usernames[i];
      }
    },
    complete: function(data, textStatus, jqXHR) {
      for (var i = 0; i < ids_.length; i++) {
        users_[i] = "아이디: " + ids_[i] + ", 회원명: " + usernames_[i];
        users[users_[i]] = null;
      }
      console.log(users);
...
function add_plant() {
  ...
  userId_autucomplete = userId_autucomplete
    .substring(0, userId_autucomplete.indexOf(","))
    .replace("아이디: ", "");
  ...
  var plantFactory = $("#plantFactory").val();
  if (
    $("#userId_autucomplete").val().length == 0 ||
    $("#plantName").val().length == 0 ||
    $("#harvestDue").val().length == 0 ||
    $("#plantPosition").val().length == 0 ||
    $("#plantGateway").val().length == 0 ||
    $("#plantSpecies").val().length == 0
  ) {
    alert("정확히 입력하세요");
  } else {
    loading_active();
    $.ajax({
      type: "POST",
      url: "http://www.nextfarmer.net/plant/addPlant",
      headers: {
        userSeq: userSeq
      },
      data: {
