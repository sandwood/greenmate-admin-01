...
function get_numberof_plants(){
	$.ajax({
		type: "POST",  
		url: 'http://13.124.207.154:3000/plant/getAllPlants',
		data:{
			category: 'all'
		}
			...
			for(var i=0; i<data.plants.length; i++){
				...
				$('#plant_').append(`
					<tr class="plant_">
						...
						<td id="plant_manager`+position+`">`+manager+`</td>
						<td id="plant_username`+position+`">`+username+`</td>
...
function start_plants_search(){
	...
	$.ajax({
		type: "POST",  
		url: 'http://13.124.207.154:3000/search/searchPlant',
		data:{
			keyword: keyword_,
			page: 1,
			pageNumber: 5
...
function start_plant_mine(){
	...
	$.ajax({
		type: "POST",  
		url: 'http://13.124.207.154:3000/manager/managingPlants',
		headers: {
			'userSeq': userSeq
		},
		data:{
...
function goto_inner_plant(plantId){
	...
	$.ajax({
		type: "POST",  
		url: 'http://13.124.207.154:3000/manager/plantDetail',
		data:{
			plantId: plantId
		},
		success: function( data, textStatus, jqXHR ){
...
function delete_plant(){
	...
	if(del == true){
		...
		$.ajax({
			type: "PUT",  
			url: 'http://13.124.207.154:3000/delete/deletePlant',
			headers:{
				userSeq: userSeq
			},
			data:{
	...
	$('.plant_checkbox').click(function(){
		...
		$('#qr_checker').append(`
			<div class="chip" id="qr_check_`+target+`">
				...
				<span class="plant_manager_chip" style="display:none">`+seller+`</span>
				<span class="plant_username_chip" style="display:none">`+buyer+`</span>
			</div>
		`)
	})
}
function get_userIds(){
	...
	$.ajax({
		type: "POST",  
		url: 'http://13.124.207.154:3000/search/searchUser',
		headers: {
			userSeq: userSeq
		},
		data:{
...
function add_plant(){
    ...
    userId_autucomplete = userId_autucomplete.substring(0, userId_autucomplete.indexOf(',')).replace('아이디: ','');
	...
	if($('#userId_autucomplete').val().length == 0 || $('#plantName').val().length == 0 || $('#harvestDue').val().length == 0 || $('#plantPosition').val().length == 0 || $('#plantGateway').val().length == 0 || $('#plantSpecies').val().length == 0){
		...
		$.ajax({
			type: "POST",  
			url: 'http://13.124.207.154:3000/plant/addPlant',
			headers: {
				userSeq: userSeq
			},
			data:{
