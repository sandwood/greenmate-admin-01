function start_qa_pages() {
  ...
  $.ajax({
    type: "POST",
    url: "http://www.nextfarmer.net/question/getQuestions",
    data: {
      page: 1,
      pageNumber: 5
    }
    ...
    for (var i = 0; i < data.questions.length; i++) {
      ...
      $("#qa_").append(
        `
          ...
          `</td>
						<td>` +
          data.questions[i].numOfComment +
          `</td>
						<td>` +
          data.questions[i].numOfManagerReply +
          `</td>
					</tr>`
      );
    }
...
function start_qa_no_a() {
  ...
  $.ajax({
    type: "POST",
    url: "http://www.nextfarmer.net/question/getUnsolved",
    data: {
      page: 1,
      pageNumber: 5
    }
...
function start_qa_search() {
  ...
  $.ajax({
    type: "POST",
    url: "http://www.nextfarmer.net/search/searchQuestion",
    data: {
      keyword: keyword_,
      page: 1,
      pageNumber: 5
...
function goto_inner_qa(id_) {
  ...
  $.ajax({
    type: "POST",
    url: "http://www.nextfarmer.net/question/getQuestionDetail",
    data: {
      questionId: id_
    }
  }).success(function(data, textStatus, jqXHR) {
    ...
    for (var i = 0; i < data.comments.length; i++) {
      ...
      if (data.comments[i].writer == 0) {
        $("#memeber_qa_comment_start").append(
          `
            ...
            `<i class="material-icons grey-text lighten-2">not_interested</i></td>
						<td class="comment_date">` +
            data.comments[i].published_date +
            `</td>
					</tr>`
        );
      }
      if (data.comments[i].writer == 1) {
        $("#memeber_qa_comment_start").append(
          `
					<tr class="comments_">
						<td class="comment_name green-text">` +
            data.comments[i].username +
            `:</td>
						<td class="comment_content">` +
            data.comments[i].comment +
...
function memeber_qa_add_comment() {
  ...
  $.ajax({
    url: "http://www.nextfarmer.net/question/addComment",
    headers: {
      userSeq: userSeq
    },
    type: "POST",
...
function delete_question() {
  ...
  if (del == true) {
    var qid = $("#hidden_id").html();
    $.ajax({
      url: "http://www.nextfarmer.net/delete/deleteQuestion/",
      headers: {
        userSeq: userSeq
      },
      type: "PUT",
