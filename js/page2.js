...
function get_numberof_guides(){
	$.ajax({
		type: "POST",  
		url: 'http://13.124.207.154:3000/manager/getGuideList',
		data:{
			category: 'all'
		}
...
function start_guide_search(){
	...
	$.ajax({
		type: "POST",  
		url: 'http://13.124.207.154:3000/search/searchGuide',
		data:{
			keyword: keyword_,
			page: 1,
			pageNumber: 5
...
function guide_block(){
	...
	$.ajax({
		url: 'http://13.124.207.154:3000/manager/guideOnOff/',
		headers: {
			guideID: qid
		},
		type: "GET"
...
function goto_inner_guide(id_){
	...
	$.ajax({
		...
		complete: function(){
			...
			$.ajax({
				type: "POST",  
				url: 'http://13.124.207.154:3000/guide/getGuideDetail',
				data:{
					guideId: id_
				},
				success: function( data, textStatus, jqXHR ){
...
function modify_guide(){
	...
	$.ajax({
		type: "PUT",  
		url: 'http://13.124.207.154:3000/manager/modifyGuide',
		headers:{
			guideId: id_
		},
		data:{
...
function delete_guide(){
	...
	if(del == true){
		...
		$.ajax({
			type: "GET",  
			url: 'http://13.124.207.154:3000/manager/deleteGuide',
			headers:{
				guideId: id_
			},
			success: function( data, textStatus, jqXHR ){
...
function upload_img(target){
	...
	if(file.name.length < 1) {}
		...
		$.ajax({
			type: "POST",  
			url: 'http://13.124.207.154:3000/uploadImage',
...
function upload_guide(){
	...
	if(title.length == '' || category.length == '' || species == null){
		...
		$.ajax({
			type: "POST",  
			url: 'http://13.124.207.154:3000/manager/addGuide',
			data:{
				title: title,
				species: category,
				category: species,
