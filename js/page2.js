...
function get_numberof_guides() {
  $.ajax({
    type: "POST",
    url: "http://www.nextfarmer.net/manager/getGuideList",
    data: {
      category: "all"
    }
  ...
  $.ajax({
    ...
    success: function(data, textStatus, jqXHR) {
      ...
      for (var i = 0; i < data.guideList.length; i++) {
        ...
        if (block == 1) {
          block_string =
            '<input type="checkbox" disabled="disabled" id="guide5"><label for="guide5"></label>';
        } else if (block == 0) {
          block_string =
            '<input type="checkbox" checked="checked" disabled="disabled" id="guide5"><label for="guide5"></label>';
        }
        ...
        $("#guide_").append(
          `
            ...
            `">
						<td>` +
            data.guideList[i].writer +
            `</td>
						<td width="320px" class="truncate">` +
            data.guideList[i].title +
            `</td>
						<td class="date_">` +
            new_date +
            `</td>
...
function start_guide_search() {
  ...
  $.ajax({
    type: "POST",
    url: "http://www.nextfarmer.net/search/searchGuide",
    data: {
      keyword: keyword_,
      page: 1,
      pageNumber: 5
...
function guide_block() {
  ...
  $.ajax({
    url: "http://www.nextfarmer.net/manager/guideOnOff/",
    headers: {
      guideID: qid
    },
    type: "GET"
...
function goto_inner_guide(id_) {
  ...
  $.ajax({
    ...
    complete: function() {
      ...
      $.ajax({
        type: "POST",
        url: "http://www.nextfarmer.net/guide/getGuideDetail",
        data: {
          guideId: id_
        },
        success: function(data, textStatus, jqXHR) {
          ...
          for (var i = 0; i < contents.length; i++) {
            if (i == 0) {
              $("#add_guide").append(
                `
							<div id="guide_contents_` +
                  i +
                  `" class="row guide_contents_wrapper">
								<div class="col s12">
									<div class="additional_guide_pics" id="additional_guide_pics_` +
                  i +
                  `">
										<img class="guide_contents_pics_" src="` +
                  contents[i].picUrl +
                  `" id="guide_pics_` +
                  i +
                  `" alt="">
								...
								<div class="col s12">
									<form id="modify_form_` +
                  i +
                  `" method="POST" enctype="multipart/form-data">
										<div class="file-field input-field">
											<div class="btn">
												<span>파일</span>
												<input class="guide_add_file" id="guide_file_` +
                  i +
                  `" type="file" name="file` +
                  i +
                  `[]" accept="image/*" >
											...
											<div class="file-path-wrapper">
												<input id="guide_file_url_` +
                  i +
                  `" class="file-path" type="text" placeholder="파일을 선택해주세요">
											</div>
										</div>
									</form>
								</div>
								...
								<div class="input-field col s12">
									<textarea id="guide_content_` +
                  i +
                  `" class="materialize-textarea">` +
                  contents[i].text +
                  `</textarea>
									<label for="guide_content_` +
                  i +
                  `">가이드 내용</label>
								</div>
							</div>
...
function modify_guide() {
  ...
  for (var i = 0; i < l; i++) {
    ...
    content_object.picUrl = $(
      "#additional_guide_pics_" + i + " .guide_contents_pics_"
    ).attr("src");
  ...
  $.ajax({
    type: "PUT",
    url: "http://www.nextfarmer.net/manager/modifyGuide",
    headers: {
      guideId: id_
    },
    data: {
...
function delete_guide() {
  ...
  if (del == true) {
    ...
    $.ajax({
      type: "GET",
      url: "http://www.nextfarmer.net/manager/deleteGuide",
      headers: {
        guideId: id_
      },
      success: function(data, textStatus, jqXHR) {
...
function upload_img(target) {
  ...
  var target_pic_wrapper = $(target)
    .closest(".guide_contents_wrapper")
		.find(".additional_guide_pics");
		console.log("target_pic_wrapper: ", target_pic_wrapper);
  ...
  if (file.name.length < 1) {
    ...
    console.log($("#" + target_form)[0]);
    $.ajax({
      type: "POST",
      url: "http://www.nextfarmer.net/uploadImage",
      ...
      complete: function() {
				...
				setTimeout(function(){
					for (var i = 0; i < pic_url.length; i++) {
						$(target_pic_wrapper).append(
							`<img class="guide_contents_pics_" src="` + pic_url[i] + `" alt="">`
						);
					}
					loading_active();
				}, 1000)
...
function upload_guide() {
  ...
  for (var i = 0; i < l; i++) {
    var content_object = new Object();
    content_object.picUrl = $(
      "#additional_guide_pics2_" + i + " .guide_contents_pics_"
    ).attr("src");
    content_object.text = $("#guide_content_" + i).val();
  ...
  if (title.length == "" || category.length == "" || species == null) {
    ...
    $.ajax({
      type: "POST",
      url: "http://www.nextfarmer.net/manager/addGuide",
      data: {
        title: title,
        species: category,
        category: species,
